#!/usr/bin/env python3
"""Re-extract titles, examples, molecular structures, and full claim1 for contrast patents"""
import sys, json, re, time, subprocess
sys.stdout.reconfigure(line_buffering=True)

REEXTRACT_SCRIPT = r'''#!/usr/bin/env python3
...
# Write re-extraction script
script_path = '/data/.hermes/skills/research/patent-playwright-scraper/reports/reextract_patent.py'
with open(script_path, 'w', encoding='utf-8') as f:
    f.write(REEXTRACT_SCRIPT)

# Patents to re-extract (all 13 US)
PIDS = [
...
for i, pid in enumerate(PIDS):
    print(f"\n[{i+1}/{len(PIDS)}] Re-extracting {pid}...")
    try:
        proc = subprocess.run(
            ...
            for line in proc.stdout.strip().split('\n'):
                if line.startswith('{'):
                    try:
                        ...
                        print(f"  mol_structs: {len(data.get('molecular_structures',[]))}")
                        print(f"  phys_data: {data.get('phys_data',[])}")
                ...
                all_results.append({'patent_id': pid, 'success': False, 'error': 'No JSON'})
        else:
...
# Save
out_path = '/data/.hermes/skills/research/patent-playwright-scraper/reports/contrast_reextract_all.json'
with open(out_path, 'w', encoding='utf-8') as f:
    json.dump(all_results, f, ensure_ascii=False, indent=2)

ok_count = sum(1 for r in all_results if r.get('success'))
print(f"\nRe-extraction complete: {ok_count}/{len(all_results)} successful")
